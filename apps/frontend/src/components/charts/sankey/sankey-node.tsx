...
export interface SankeyNodeProps {
  ...
  /** Formats the value shown under the node name (default: number + " sessions") */
...
const defaultFormatNodeValue = (value: number) =>
  `${value.toLocaleString('es-UY')} sessions`;
...
export function SankeyNode({
  ...
  formatNodeValue = defaultFormatNodeValue,
}: SankeyNodeProps) {
  const {
    nodes,
    links,
  ...
  const getColor = useCallback(
    (
      ...
      return defaultColors[index % defaultColors.length] ?? 'hsl(var(--chart-1))';
    },
      ...
      {nodes.map((node, index) => {
        ...
        return (
          <AnimatedNode
            ...
            rx={lineCap}
            formatNodeValue={formatNodeValue}
